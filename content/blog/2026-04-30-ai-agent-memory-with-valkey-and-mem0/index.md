...
There are two broad ways to add memory to agents, and the main distinction is scope: who needs access to the memory, where it must be available, how long it should persist, and whether it must be queried dynamically. Stable, shared guidance that is known before a session begins belongs in file-based context and is typically used for coding conventions, build commands, architecture notes, review checklists, and other persistent project instructions. [CLAUDE.md](https://code.claude.com/docs/en/memory), [Cursor Rules](https://cursor.com/docs/context/rules), and [OpenAI AGENTS.md](https://developers.openai.com/codex/guides/agents-md) are examples of this pattern. Claude Code uses CLAUDE.md for persistent instructions read at session start, Cursor Rules provides persistent project, team, and user rules, and OpenAI Codex reads AGENTS.md before doing any work.

External memory is the better fit for dynamic, mutable memory learned during execution. For example, user preferences, task checkpoints, past supporting context, and artifact handoffs between collaborating agents are learned at runtime, tied to a specific scope (such as user, agent, or run_id) and need to remain available across prompts, sessions, services, or devices. An external memory layer makes that practical by persisting runtime state, scoping it with identifiers and supporting centralized access control, auditing, and managed deletion.

Long-running and always-on agents highlight the need for [persistent external memory](https://developers.openai.com/api/docs/guides/agents/running-agents). These systems operate across [many context windows](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents), and often multiple sessions or collaborating agents, so the active prompt alone cannot serve as the system's memory. This is where an external memory layer like Valkey becomes useful, persisting runtime state outside the prompt and making it retrievable on demand across long-running workflows.
...
FT.CREATE {collection_name} ON HASH PREFIX 1 {prefix} SCHEMA
    ...
    memory_id TAG hash TAG agent_id TAG run_id TAG user_id TAG metadata TAG
    created_at NUMERIC updated_at NUMERIC
```

The `memory` attribute stores the extracted fact in natural language, and the `embedding` attribute stores its vector representation. Scope attributes such as `user_id`, `agent_id`, and `run_id` let the system narrow retrieval to the right user, agent, or session before running vector search. Metadata attributes such as `updated_at` and `created_at` timestamps support filtering, updates, and lifecycle management.
...
Finally, Mem0 writes each candidate memory to Valkey with a single HSET.
```text
HSET mem0:{memory_id}
...
This section focuses on long-term memory, which puts the sharpest pressure on the storage layer. Unlike retrieval-augmented generation (RAG), which reads from a mostly static corpus, long-term memory is a live, mutation-heavy workload. A single interaction can produce several candidate memories, each triggering its own scoped lookup and an add, update, delete, or no-op decision. In practice, that means multiple small reads, searches, and writes per turn. The storage layer has to support the four requirements below so the agent can continuously learn, update, and reuse context.
