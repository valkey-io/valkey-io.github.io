...
```
+select +@write ~* -@dangerous -flushall -flushdb -restore -restore-asking
```

So the replication user (specified by the config parameter `primaryuser`) will need the following ACL permissions:

```
+psync +replconf +ping +cluster|syncslots +select +@write ~* -@dangerous -flushall -flushdb -restore -restore-asking
```
...
For ASM to work correctly, the replication user therefore needs `+psync +replconf +ping +cluster|syncslots +select +@write ~* -@dangerous -flushall -flushdb -restore -restore-asking`.
