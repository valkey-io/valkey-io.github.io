...
Note that the order of ACL rules matters. For example, `PSYNC` is a dangerous command, so the ACL rules `+psync -@dangerous` will prevent the user from running `PSYNC`, while `-@dangerous +psync` will forbid the user from running any dangerous commands except `PSYNC`.
...
For ASM to work correctly, the replication user therefore needs `+@write ~* -@dangerous +ping +select +psync +replconf +cluster|syncslots -flushall -flushdb -restore -restore-asking`.
