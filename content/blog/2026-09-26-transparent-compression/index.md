...
The configuration change shown below enables compression with the LZ4 backend and delivers memory savings of 28.1% with minimal throughput and latency impact in our benchmarks. Most configurations measured between 0.95x and 1.07x of uncompressed SET throughput on the benchmarked 2KB JSON strings workload. (The default backend is zstd, which trades some write throughput for roughly double the memory saving. More on choosing between them below.)
...
1. Is the value larger than the minimum size threshold (default: 64 bytes)? If not, send it uncompressed as the 5-byte header overhead would negate any savings and small values are unlikely to be compressible.
...
### Compatibility:
Compression is **not** compatible with commands that read or manipulate string data on the server. The server only ever sees the compressed bytes, so these commands operate on the compressed representation rather than your original value. Valkey GLIDE will return an error if a compression-enabled client attempts to use these commands.
...
If you rely on these commands, keep the affected keys uncompressed by splitting them onto a separate client without compression configured (see Best Practices below).


## Memory Savings

The headline memory savings figures are the result of compressing 10,000 ~2KB JSON strings and measuring Valkey's used_memory metric:
...
Zstd saves nearly twice as much memory as LZ4 on this JSON workload. But ~2KB JSON strings are just one scenario and not representative of the typical size of data you might be working with. In practice, you're caching all kinds of data at all kinds of sizes. Below are the results for JSON, HTML, and session data across a variety of value sizes to show what savings look like across representative synthetic workloads.

### Memory Savings by Payload Type and Value Size

| Payload Type | Avg Bytes | zstd Savings | LZ4 Savings |
|-----------|-----------|-------------|------------|
| JSON | 97 | 0.9% | 0.0% |
| JSON | 461 | 28.5% | 14.6% |
...
**Value size matters more than payload type.** Below ~100 bytes, neither algorithm saves meaningful memory and the 5-byte header overhead and Valkey's per-key metadata dominate. Above ~500 bytes, both algorithms deliver substantial savings across all payload types.
...
**zstd consistently beats LZ4 on compression ratio.** Across every payload type and size, zstd saves more memory. The gap is widest on highly compressible data and narrowest on small or low-redundancy data.
...
The cost lives on the write path. Compressing a value takes real CPU time, and with batching that cost is multiplied by the number of values in the batch. At batch=1 the effect is barely visible and all three backends show similar p50 SET latency, around 0.33ms at 10 goroutines. At batch=10 with 10 goroutines, baseline batch latency rises to ~0.42ms, zstd to ~0.64ms, and LZ4 stays at ~0.46ms. The zstd cost scales with the number of values compressed per batch, while LZ4 remains close to baseline throughout.

The read path is the opposite. Decompression is far cheaper than compression, and every GET moves a payload roughly half the size over the network. At low concurrency that saving is small, but as concurrency grows and queueing dominates latency, moving half the bytes wins outright: zstd GET p95 beat the uncompressed baseline in 30 of 40 configurations tested and stayed within 2% in all but two. At 1000 goroutines the gap is dramatic and batch=10 GET p95 drops from ~95ms uncompressed to ~69ms with zstd, and batch=50 from ~71ms to ~51ms.
...
**Start with LZ4** if your workload is write-heavy and you want compression to be effectively invisible. Switch to zstd for maximum memory savings if your write path can handle the compression cost. Read-heavy workloads often see tail latency improve rather than degrade. The savings you'll see depend heavily on your payload type and value size. HTML compresses best, session data compresses least, and anything under 100 bytes isn't worth compressing. Skip compression entirely for already-compressed data (images, video, pre-compressed content).
...
For value sizes:
- **Under 100 bytes**: Skip compression. Neither algorithm saves meaningful memory at this size and the 5-byte header overhead and Valkey's per-key metadata dominate.
- **100–500 bytes**: Savings vary by payload type. HTML compresses well even at ~193 bytes (21.3% zstd / 10.4% LZ4). Worth enabling with zstd; test with LZ4.
- **500–1,000 bytes**: Solid savings across all payload types. Expect 17–50% with zstd and 0–29% with LZ4 depending on payload type.
...
We'd love to hear about your experience with compression. Let us know what payload types you're compressing, what savings you're seeing, and what would make the feature more useful. Join the conversation on [GitHub Discussions](https://github.com/valkey-io/valkey-glide/discussions).
