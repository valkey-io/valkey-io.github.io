...
- **Replication of Slot Migration States**: All `CLUSTER SETSLOT` commands are
  ...
  reduces the chance of unavailability in the event the primary fails, as the replica has the most up to date information about the state of the shard. 
    New Replicas also automatically inherit the state from the primary, with no additional input needed from an operator.
