...
Replication traffic efficiency was also improved in 8.1 by offloading the replication stream reading, and the replica writes to the I/O thread pool. Replica reads were kept in the main thread only to reduce the complexity and because read traffic from replicas is negligible.
