...
This new design reduces by one the number of memory accesses required to lookup an entry when there are no collisions, but it reduces a lot more memory accesses to lookup an entry when there are key mapping collisions. It also reduces the memory footprint by a lot when there are collisions.
