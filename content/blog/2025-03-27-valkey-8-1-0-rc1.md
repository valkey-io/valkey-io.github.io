...
The new hashtable implementation is a complete rewrite of the previous hashtable implementation, which reduces the memory usage by roughly 20 bytes per key-value pair and improves the latency and CPU usage by roughly 10% for instances without I/O threading.
...
It also reduces the memory footprint when there are collisions. With 3 keys mapping to the same bucket, the new design is already using less 8 bytes than the old design, which requires 72 bytes of memory (3 `dictEntry` objects). In the new design, the 64 byte bucket can store up to 7 entries, while the old design would require 168 bytes.
...
Valkey 8.1 continues to add optimisations which will greatly benefit users. Leaderboards are a popular use case for Valkey using its sorted set data type. Retrieving the specific rank of an entry on the leaderboard is done through the ZRANK operation. 

By analyzing the performance of ZRANK, a new algorithm was developed to optimise it’s performance by up to 45% depending on the size of the sorted set. 
