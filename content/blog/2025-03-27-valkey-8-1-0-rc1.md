...
You can learn more about the design and results in [the dedicated blog post about the implementation](/blogs/new-hash-table). 
...
Active Defrag has been improved to eliminate large latencies to below [1ms](https://github.com/valkey-io/valkey/pull/1242), by running a defrag cycle time of 500us every 10ms. This results in much more predictable latencies, with a dramatic reduction in tail latencies.

An anti-starvation protection has also been introduced in the presence of long-running commands. If a slow command delays the dfrag cycle from kicking in, the defrag process will run proportionately longer to ensure that the configured CPU is achieved. Given the presence of slow commands, the proportional extra time is insignificant to latency. 
