...
are experiencing latency problems with Valkey.
...
client. Usually Valkey processing time is extremely low, in the sub microsecond
range, but there are certain conditions leading to higher latency figures.

I've little time, give me the checklist
---

The following documentation is very important in order to run Valkey in
...
Valkey provides latency monitoring capabilities that
...
the topic of Valkey and latency spikes.

Latency baseline
----------------

There is a kind of latency that is inherently part of the environment where
you run Valkey, that is the latency provided by your operating system kernel
...
it is the baseline, or in other words, you won't be able to achieve a Valkey
...
Clients connect to Valkey using a TCP/IP connection or a Unix domain connection.
...
The consequence is even if Valkey processes most commands in sub microsecond
...
that purpose. A number of commands also support
...
+ Valkey supports Lua server-side scripting to cover cases that are not suitable
...
vanilla Valkey is not really suitable to be bound on a **single** CPU core.
Valkey can fork background tasks that can be extremely CPU consuming
...
values composed of many elements, or you should run a replica using Valkey
...
main Valkey process. If it is high while the traffic is not, it is usually
...
`KEYS`, as documented in the Valkey documentation, should only be used for
...
In order to generate the RDB file in background, or to rewrite the Append Only File if AOF persistence is enabled, Valkey has to fork background processes.
...
per page of the address space of the process. So a large 24 GB Valkey instance
...
Unfortunately when a Linux kernel has transparent huge pages enabled, Valkey
...
If a Valkey page is moved by the kernel from the memory to the swap file, when
the data stored in this memory page is used by Valkey (for example accessing
a key stored into this memory page) the kernel will stop the Valkey process
...
and will result into anomalous latency experienced by Valkey clients.

The kernel relocates Valkey memory pages on disk mainly because of three reasons:
...
this problem is simply Valkey using more memory than is available.
* The Valkey instance data set, or part of the data set, is mostly completely idle
...
includes Valkey RDB and/or AOF background threads which can produce large files.
...
the Valkey process (assuming you are using Linux 2.6.16 or newer).
...
different memory maps of our Valkey process (The memory layout of a process
...
site running Valkey and serving hundreds of users every second) there are a
...
Valkey instance you can further verify it using the **vmstat** command:
...
If your latency problem is due to Valkey memory being swapped on disk you need
to lower the memory pressure in your system, either adding more RAM if Valkey
...
Another source of latency is due to the Append Only File support on Valkey.
...
doing I/O. For this reason when possible Valkey does the fdatasync(2) call
...
* When appendfsync is set to the value of **no** Valkey performs no fsync.
...
cope with the speed at which Valkey is receiving data, however this is
uncommon if the disk is not seriously slowed down by other processes doing
I/O.

* When appendfsync is set to the value of **everysec** Valkey performs a
fsync every second. It uses a different thread, and if the fsync is still
in progress Valkey uses a buffer to delay the write(2) call up to two seconds
(since write would block on Linux if a fsync is in progress against the
same file). However if the fsync is taking too long Valkey will eventually
...
(actually Valkey will try to cluster many commands executed at the same time
...
Most Valkey users will use either the **no** or **everysec** setting for the
...
well with the append only file if the disk is spare as Valkey writes
...
Valkey in the main thread. With the above command you'll not see the
...
Valkey evict expired keys in two ways:
...
Given that `ACTIVE_EXPIRE_CYCLE_LOOKUPS_PER_LOOP` is set to 20 by default, and the process is performed ten times per second, usually just 200 keys per second are actively expired. This is enough to clean the DB fast enough even when already expired keys are not accessed for a long time, so that the *lazy* algorithm does not help. At the same time expiring just 200 keys per second has no effects in the latency a Valkey instance.
...
Basically this means that **if the database has many, many keys expiring in the same second, and these make up at least 25% of the current population of keys with an expire set**, Valkey can block in order to get the percentage of keys already expired below 25%.
...
Valkey software watchdog
---

The *Valkey Software Watchdog* is a debugging tool
...
normal execution of the Valkey server.
