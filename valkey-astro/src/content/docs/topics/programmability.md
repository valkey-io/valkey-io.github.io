...
* It starts accepting commands again from other clients but will reply with a BUSY error to all the clients sending normal commands. A few commands allowed in this state are those that manage the connection, control the busy state, or modify client metadata, such as `SCRIPT KILL`, `FUNCTION KILL`, `SHUTDOWN NOSAVE`, `AUTH`, `HELLO`, `QUIT`, `RESET`, `SELECT`, and various `CLIENT` sub-commands.
